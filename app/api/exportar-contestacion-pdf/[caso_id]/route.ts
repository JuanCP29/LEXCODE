...
import { generarContestacionPdf } from "@/lib/pdf/generar-contestacion-pdf";
...
export async function GET(_req: NextRequest, { params }: { params: { caso_id: string } }) {
  try {
    ...
    const buffer = await generarContestacionPdf({
    ...
    const nombre = `CONTESTACION_DDA_${caso.cedula_demandante ? `CC_${caso.cedula_demandante}` : caso.radicado}.pdf`;
    ...
    console.error("exportar-contestacion pdf:", e);
