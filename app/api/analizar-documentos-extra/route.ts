...
- PRETENSION PRINCIPAL vs SUBSIDIARIA (IMPORTANTE): cuando la demanda formule una pretension PRINCIPAL y otra(s) SUBSIDIARIA(S) o "en subsidio", clasifica SIEMPRE por la PRINCIPAL (la que se pide en primer lugar), no por la subsidiaria, aunque el acto administrativo ancla haya negado la subsidiaria. En particular, la INDEMNIZACION SUSTITUTIVA suele pedirse en subsidio de una pension de vejez: si la pretension principal es la PENSION DE VEJEZ, la clase corresponde a esa pension (p. ej. LEY 100 DE 1993, DECRETO 758 DE 1990), y NO "INDEMNIZACION SUSTITUTIVA".`;
...
C) CUANTIA -> campo "cuantia". ${REGLA_CUANTIA}

D) NORMAS Y JURISPRUDENCIA -> campo "normas". ${REGLA_NORMAS}

E) PROBLEMA JURIDICO -> campo "problema_juridico". Usa la JURISDICCION indicada arriba si se indico. ${REGLA_PROBLEMA_JURIDICO}
...
Devuelve UNICAMENTE un objeto JSON valido con esta forma exacta (sin texto adicional):
{
  ...
  "suggestions": {
    ...
    "cuantia": "${REGLA_CUANTIA}",
    "normas": "${REGLA_NORMAS}",
    "problema_juridico": "${REGLA_PROBLEMA_JURIDICO}",
