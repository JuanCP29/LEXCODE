 ...
 * Es defensivo: cualquier fallo devuelve null (no rompe el análisis).
 */
function extraerIdentificadoresSentencias(texto: string): string[] {
  if (!texto) return [];
...
async function sugerirJurisprudencia(
  ...
  try {
    ...
    if (caracteresExtraidos < 200 || trasladoChars < 200) {
      ...
      const jurisprudencia = await sugerirJurisprudencia(supabase, anthropic, {
        pretension: vision.pretension, normasText: vision.normas,
      });
...
Devuelve UNICAMENTE un objeto JSON valido con esta forma exacta (sin texto adicional):
{
  ...
  "suggestions": {
    ...
    // Sección 9 (Jurisprudencia): elige la sentencia más relevante de la Sección 4,
    // la busca en el repositorio y la resume (o null si no hay fuente).
    suggestions.jurisprudencia = await sugerirJurisprudencia(supabase, anthropic, {
      pretension: clasif.pretension,
      normasText: typeof suggestions.normas === "string" ? suggestions.normas : null,
      demandaTexto: textoCompleto,
    });
