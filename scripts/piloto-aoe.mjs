/**
 * Piloto de extracción del histórico AOE (Fase histórica, Paso 8 del plan).
 * Dedup por cédula, muestra variada (~N repartida entre los 18 repartos), extrae la ficha
 * estructurada de cada AOE con Sonnet (texto; visión si está escaneado) y la inserta en aoe_casos.
 *   node scripts/piloto-aoe.mjs [N]        (N = tamaño del piloto, default 40)
 */
...
// Saneamiento de fuentes_citadas: deja solo fuentes doctrinales internas + jurisprudencia.
...
const RE_PLACEHDR = /^\s*(no\s+aplica|ninguna?|n\.?\/?a\.?|sin\s+(fuentes?|citas?)|no\s+(cita|registra|aplica|existe))\s*$/i;
function saneaFuentes(v) {
  if (!Array.isArray(v)) return [];
  return v.filter((f) => {
    if (!f || typeof f !== "object") return false;
    const tipo = String(f.tipo ?? "").toLowerCase().trim();
    if (!TIPOS_FUENTE_OK.includes(tipo)) return false;             // descarta norma/acto/otros
    const id = String(f.identificacion ?? "").trim();
    const frag = String(f.fragmento ?? "").trim();
    if (RE_PLACEHDR.test(id) || (!id && RE_PLACEHDR.test(frag))) return false;   // placeholders
    if (tipo !== "jurisprudencia" && RE_ACTO_EXPEDIENTE.test(id)) return false;  // acto del expediente
    if (!id && !frag) return false;                                 // entrada vacía
    return true;
  }).slice(0, 40);
}
...
// 1. Dedup por cédula + agrupar por reparto.
...
for (const ruta of lines) {
  const c = ruta.match(/CC_(\d+)_AOE/i)?.[1]; const rep = ruta.match(/REPARTO (\d+)/i)?.[1] || "?";
  if (c && !porCedula.has(c)) porCedula.set(c, { ruta, reparto: rep });
}
// Excluir cédulas ya extraídas (para validar sobre casos NUEVOS). Se puede desactivar con FRESH=0.
const fresh = process.env.FRESH !== "0";
let yaExtraidas = new Set();
if (fresh) {
  const { data: prev } = await sb.from("aoe_casos").select("cedula");
  yaExtraidas = new Set((prev ?? []).map((r) => String(r.cedula)));
  for (const c of yaExtraidas) porCedula.delete(c);
}
console.log(`Casos distintos: ${porCedula.size} nuevos (excluidas ${yaExtraidas.size} ya extraídas). Piloto: ${N}.`);

// 2. Muestra variada y ALEATORIA dentro de cada reparto.
const shuffle = (a) => { for (let i=a.length-1;i>0;i--){ const j=Math.floor(Math.random()*(i+1)); [a[i],a[j]]=[a[j],a[i]]; } return a; };
const porRep = new Map();
for (const [ced, v] of porCedula) { if (!porRep.has(v.reparto)) porRep.set(v.reparto, []); porRep.get(v.reparto).push({ ced, ...v }); }
const reps = [...porRep.keys()].sort();
const porRepN = Math.max(1, Math.ceil(N / reps.length));
const muestra = [];
for (const r of reps) { const g = shuffle(porRep.get(r).slice()); for (let i=0;i<g.length && i<porRepN && muestra.length<N;i++) muestra.push(g[i]); }

// 3. Procesar.
let ok=0, fail=0, totalDescartadas=0; const stats = {};
for (const item of muestra) {
  try {
    const buf = fs.readFileSync(item.ruta);
    const texto = await extraerTexto(buf);
    const estado = texto.length >= 120 ? "ok" : "ocr";
    const f = await llamar(texto, buf);
    if (!f) { console.log(`✗ SIN FICHA CC ${item.ced}`); fail++; continue; }
    const row = {
      ...
      problema_juridico: f.problema_juridico?.trim() || null,
      causal_decision: f.causal_decision?.trim() || null,
      requisitos_examinados: arr(f.requisitos_examinados),
      hechos_relevantes: f.hechos_relevantes?.trim() || null,
      investigacion_administrativa: f.investigacion_administrativa?.trim() || null,
      normas_citadas: arr(f.normas_citadas),
      ...
      excepciones: arr(f.excepciones),
      estado_texto: estado, confianza: f.confianza ? String(f.confianza).toLowerCase().trim() : null,
      extraido_modelo: "claude-sonnet-4-6",
    };
    const { error } = await sb.from("aoe_casos").upsert(row, { onConflict: "cedula,radicado" });
    if (error) { console.log(`✗ UPSERT CC ${item.ced}: ${error.message}`); fail++; continue; }
    stats[row.tipo_prestacion] = (stats[row.tipo_prestacion]||0)+1;
    const descartadas = (Array.isArray(f.fuentes_citadas)?f.fuentes_citadas.length:0) - row.fuentes_citadas.length;
    if (descartadas > 0) totalDescartadas += descartadas;
    const fdump = row.fuentes_citadas.map((x)=>`${x.tipo}:${String(x.identificacion||"").slice(0,24)}`).join(" | ") || "—";
    console.log(`✓ R${item.reparto} CC ${item.ced.padEnd(10)} | ${row.tipo_prestacion.padEnd(13)} | ${row.sentido_decision||"?"} | fuentes:${row.fuentes_citadas.length}${descartadas>0?`(-${descartadas})`:""} normas:${row.normas_citadas.length} | ${estado} | conf:${row.confianza}\n      fuentes: ${fdump}`);
    ok++;
  } catch (e) { console.log(`✗ ERROR CC ${item.ced}: ${e?.message ?? e}`); fail++; }
}
console.log(`\nHecho. OK:${ok} fail:${fail}. Fuentes saneadas descartadas: ${totalDescartadas}. Por prestación: ${JSON.stringify(stats)}`);
